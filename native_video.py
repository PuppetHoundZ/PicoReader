...
# same as the rest of this file -- not muOS-specific. Used for the
# +-10min big-skip buttons (v26.07.21.24: now bound to L2/R2, was
# originally L1/R1 -- see _translate_loop for the current mapping).
KEY_PAGEUP = 104
KEY_PAGEDOWN = 109
...
KEY_A_LETTER = 30
# v26.07.21.24 (Kaleb's request): brightness control, but NOT the OS
# backlight -- mpv's own `brightness` video-filter property, applied
# in software to the decoded frame before it's ever handed to the
# display, live-adjustable while playing via input.conf's "add
# brightness N" command. ffplay has no equivalent runtime capability
# (only a launch-time -vf eq=brightness flag, not adjustable while
# playing), so these are safe no-ops there -- not bound to anything in
# ffplay's documented default keymap. 'i'/'o' chosen simply because
# they're plain, unused ASCII letters -- no mnemonic significance,
# just guaranteed not to collide with anything already bound here.
KEY_BRIGHTNESS_DOWN = 23  # 'i'
KEY_BRIGHTNESS_UP = 24    # 'o'
    ...
    def create(cls):
        ...
        try:
            ...
            for key in (KEY_SPACE, KEY_Q, KEY_LEFT, KEY_RIGHT, KEY_UP, KEY_DOWN,
                        KEY_PAGEUP, KEY_PAGEDOWN, KEY_T, KEY_A_LETTER,
                        KEY_BRIGHTNESS_DOWN, KEY_BRIGHTNESS_UP):
...
# (script/var/func.sh) reimplemented here via subprocess since Python
# has no access to those shell functions directly.
...
def _write_config_var(path, value):
    try:
...
def _restart_hotkey_daemon():
    """Reimplements muOS's own HOTKEY restart (script/var/func.sh) --
    kill any running muhotkey/hotkey.sh, then launch a fresh one via
    setsid so it re-reads the now-changed idle_sleep/idle_display
    values. Best-effort: if this fails (e.g. off-device, or muOS's own
    process names ever change), the config values are still written
    correctly, they just won't take effect until something else
    restarts the daemon (a normal settings-menu save would also do it)
...
def suppress_idle_display():
    """Called once, right before video playback starts. Returns
    ...
    orig_sleep = _read_config_var(_IDLE_SLEEP_CFG_PATH)
    orig_display = _read_config_var(_IDLE_DISPLAY_CFG_PATH)
    try:
        open(_CAFFEINE_PATH, "w").close()
    except OSError:
        pass
    if orig_sleep is not None:
        _write_config_var(_IDLE_SLEEP_CFG_PATH, "0")
    if orig_display is not None:
        _write_config_var(_IDLE_DISPLAY_CFG_PATH, "0")
    if orig_sleep is not None or orig_display is not None:
        _restart_hotkey_daemon()
    return orig_sleep, orig_display


def restore_idle_display(orig_sleep, orig_display):
    """Called once, when video playback ends -- MUST be called even if
    playback ended abnormally (crash, early quit), or the user's real
    idle settings stay disabled. See play_jw_video()'s own try/finally
    for where this is guaranteed to run."""
    try:
        os.remove(_CAFFEINE_PATH)
    except OSError:
        pass
    if orig_sleep is not None:
        _write_config_var(_IDLE_SLEEP_CFG_PATH, orig_sleep)
    if orig_display is not None:
        _write_config_var(_IDLE_DISPLAY_CFG_PATH, orig_display)
    if orig_sleep is not None or orig_display is not None:
        _restart_hotkey_daemon()
    ...
    while not stop_event.is_set():
        while sdl.SDL_PollEvent(ctypes.byref(ev_buf)) != 0:
            ...
            if etype == SDL_JOYBUTTONDOWN_EV:
                ...
                # v26.07.21.24 (Kaleb's request): remapped from the
                # original v26.07.20.35 layout -- D-pad Up/Down now
                # controls brightness (below), so the +-1min seek it
                # used to do moved to L1/R1, and the +-10min skip that
                # used to live on L1/R1 moved to L2/R2. Same KEY_UP/
                # KEY_DOWN/KEY_PAGEUP/KEY_PAGEDOWN taps as before, just
                # bound to different physical buttons now -- the
                # bundled mpv input.conf's seek bindings for those keys
                # are unchanged, and ffplay's own native handling for
                # them is unchanged too. All four optional/None-safe,
                # same tolerant pattern as before.
                elif joy_l1 is not None and bev.button == joy_l1:
                    vkbd.tap(KEY_DOWN)      # L1 -- seek back ~1 min
                elif joy_r1 is not None and bev.button == joy_r1:
                    vkbd.tap(KEY_UP)        # R1 -- seek forward ~1 min
                elif joy_l2 is not None and bev.button == joy_l2:
                    vkbd.tap(KEY_PAGEDOWN)  # L2 -- skip back ~10 min
                elif joy_r2 is not None and bev.button == joy_r2:
                    vkbd.tap(KEY_PAGEUP)    # R2 -- skip forward ~10 min
                # v26.07.21.01 (Kaleb's request): Y -- toggle/cycle
                # subtitles. Optional, same tolerant None-safe pattern
                # as joy_l1/joy_r1.
                elif joy_y is not None and bev.button == joy_y:
                    vkbd.tap(KEY_T)
                # v26.07.21.02 (Kaleb's request): X -- cycle audio track.
                # Same tolerant None-safe pattern as joy_l1/joy_r1/joy_y.
                elif joy_x is not None and bev.button == joy_x:
                    vkbd.tap(KEY_A_LETTER)
                # v26.07.21.38 (Kaleb's request: Play All/Shuffle All for
                # video too). Every other button is already spoken for
                # during video playback (all 4 shoulder buttons = seek/
                # skip, D-pad = seek/brightness, Y/X = subtitle/audio
                # track) -- START and SELECT are the only two genuinely
                # unused buttons left, so they're the queue-skip controls
                # here, same as L/R filled that role for audio (which had
                # shoulder buttons free). Only meaningful when `signal`
                # is actually passed (i.e. this playback is part of a
                # play_video_queue() session) -- a plain single video
                # play passes signal=None, so these two are silent no-ops
                # then, same as every other None-safe optional button.
                elif signal is not None and joy_select is not None and bev.button == joy_select:
                    signal["skip"] = 1
                    vkbd.tap(KEY_Q)      # SELECT -- next video
                elif signal is not None and joy_start is not None and bev.button == joy_start:
                    signal["skip"] = -1
                    vkbd.tap(KEY_Q)      # START -- previous video
                ...
                # v26.07.21.24 (Kaleb's request): D-pad Up/Down was
                # +-1min seek (moved to L1/R1 above); now controls
                # mpv's own live `brightness` video property instead --
                # see KEY_BRIGHTNESS_UP/DOWN's own comment for exactly
                # what this does and doesn't affect (mpv only, software/
                # per-video, NOT the OS backlight). Safe no-op on
                # ffplay, which has no equivalent runtime capability.
                elif hv & SDL_HAT_UP:
                    vkbd.tap(KEY_BRIGHTNESS_UP)
                elif hv & SDL_HAT_DOWN:
                    vkbd.tap(KEY_BRIGHTNESS_DOWN)
...
def play_jw_video(source, is_local, sdl=None, joy_a=None, joy_b=None,
                   ...
                   signal=None, player_pref="auto", _manage_idle=True):
    ...
    `joy_l1`/`joy_r1` (+-1min seek) and `joy_l2`/`joy_r2` (+-10min skip)
    -- see _translate_loop's own comment for the full current layout.
    ...
    default audio-cycle key is '#', not 'a'. D-pad Up/Down controls
    mpv's own live `brightness` video property -- software, per-video,
    NOT the OS backlight -- via 'i'/'o', safe no-ops on ffplay (no
    runtime brightness capability there). If uinput isn't available on
    ...
    `player_pref` -- "auto" (default, behavior described above), "mpv",
    or "ffplay" for an explicit manual override. Even with an explicit
    choice, this STILL falls back to the other player if the preferred
    one genuinely isn't found on this device -- an explicit preference
    means "prefer this one", not "only this one, fail hard otherwise".
    ...
    if vkbd is not None:
        thread = threading.Thread(
            target=_translate_loop, args=(sdl, joy_a, joy_b, vkbd, stop_event, joy_l1, joy_r1, joy_y, joy_x,
                                            joy_l2, joy_r2, joy_start, joy_select, signal),
            daemon=True)
        thread.start()
...
def play_video_queue(items, start_index=0, shuffle=False, sdl=None, joy_a=None, joy_b=None,
                      ...
                      screen_h=None, player_pref="auto", on_track_change=None, resolve_source=None):
    ...
    list of videos back-to-back via play_jw_video() per item, either in
    ...
    as play_audio_queue() -- play_jw_video() is called with
    ...
    which knows the real jw_fetch.py download-folder/filename
    ...
    try:
        while 0 <= pos < len(order):
            ...
            ok, msg = play_jw_video(
                ...
                player_pref=player_pref, _manage_idle=False)
            if not ok:
...
def _audio_translate_loop(sdl, joy_a, joy_b, joy_l, joy_r, vkbd, stop_event, signal):
    """Simpler sibling of _translate_loop() (video's own) -- audio has
    no seek/subtitle/audio-track/brightness controls, just pause,
    quit-track-vs-quit-queue, and skip prev/next. `signal` is a shared
    dict {"abort": bool, "skip": int} the OUTER queue loop (see
    play_audio_queue() below) reads after each track ends: abort=True
    means stop the whole queue (B was pressed); skip=+1/-1 means the
    CURRENT track was deliberately skipped (R/L), advance/retreat the
    queue position instead of treating it as the track naturally
    finishing. Ending the current track (to let the queue loop move
    on) is done the same way for skip and quit alike -- tap Q, the
    same key both mpv and ffplay already quit on -- the only
    difference is which flag gets set first so the queue loop knows
    which case it's actually handling."""
    SDL_JoyHatEvent, SDL_JoyButtonEvent = _make_event_types()
    ev_buf = (ctypes.c_byte * 56)()
    while not stop_event.is_set():
        while sdl.SDL_PollEvent(ctypes.byref(ev_buf)) != 0:
...
def play_audio_file(url, sdl=None, joy_a=None, joy_b=None, joy_l=None, joy_r=None,
    ...
    (ok: bool, message: str | None), same contract as play_jw_video().
    ...
    play_jw_video()'s own is_local handling, rather than requiring a
    real jw.org URL.
...
def play_audio_queue(items, start_index=0, shuffle=False, sdl=None, joy_a=None, joy_b=None,
                      joy_l=None, joy_r=None, player_pref="auto", on_track_change=None,
                      resolve_source=None):
    ...
    All"). `items` is the SAME list shape jw_fetch.py's audio loaders
    already return (each with a real "_audio_url" and "title", the
    exact fields already confirmed live throughout this session).
    ...
    stopped by the user (B), matching play_jw_video()'s own "user quit
